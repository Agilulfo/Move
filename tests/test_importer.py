...
def test_import_non_existent_source(tmp_path):
    ...
    assert "Invalid Path" in result["errors"]["non-existent-path"]
...
def test_import_unsupported_single_file(tmp_path):
    ...
    with pytest.raises(InvalidGpxFile):
        import_gpx(str(source_file), str(storage_dir))
